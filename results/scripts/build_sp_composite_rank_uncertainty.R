...
category_path <- "results/projections/pitchers/sp_category_projections_2026.csv"
out_csv <- "results/projections/pitchers/sp_composite_rank_2026.csv"
out_md <- "results/projections/pitchers/top50_sp_composite_by_role.md"

draw_cap <- as.integer(Sys.getenv("UNCERTAINTY_DRAW_CAP", "0"))
set_seed <- as.integer(Sys.getenv("UNCERTAINTY_SEED", "123"))
ip_cv <- as.numeric(Sys.getenv("SP_IP_ATC_CV", "0.10"))
...
if (length(keep) == 0) {
  stop("No players with non-missing positive IP_atc in SP projections.")
}
...
if (any(is.na(eta_idx))) {
  stop("Could not align SP projection player ids to eta_pred indices.")
}
...
qs_rate <- exp(drop3(eta[, , 6, drop = FALSE]))
...
qs_count <- draw_poisson(qs_rate, ip_mat)

ERA <- (er_count / ip_mat) * 9
WHIP <- (bb_count + h_count) / ip_mat
Ks <- so_count
WQS <- w_count + qs_count
IP <- ip_mat
...
z_IP <- zscore_rows(IP)
z_WQS <- zscore_rows(WQS)

comp_draw <- (z_ERA + z_Ks + z_WHIP + z_IP + z_WQS) / 5
...
summ <- tibble(
  ...
  z_WQS_p50 = apply(z_WQS, 2, quantile, probs = 0.50, na.rm = TRUE),
...
lines <- c(
  "# Top 50 SP composite rankings by role (2026)",
  "",
  "## Method",
  "- Rank pitchers within each posterior predictive draw using draw-level SP composite z-scores.",
  "- Composite uses ERA (flipped), Ks, WHIP (flipped), IP, and W+QS.",
